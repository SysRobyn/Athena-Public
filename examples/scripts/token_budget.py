...
ON_DEMAND_FILES = [
    ...
    (".context/profile/Psychology_L1L5.md", "User profile layers and behavioral data"),
...
def main():
    ...
    print(
        f"{BOLD}{CYAN}              📊 CODEX TOKEN BUDGET REPORT                      {RESET}"
    )
    print(
        f"{CYAN}═══════════════════════════════════════════════════════════════{RESET}\n"
    )
    ...
    print(f"\n{DIM}{'─' * 63}{RESET}")
    print(
        f"{BOLD}{GREEN}  /start BOOT TOTAL{' ' * 16} │ {format_tokens(boot_total):>8} tokens{RESET}"
    )
    ...
    for rel_path, trigger in ON_DEMAND_FILES:
        ...
        if error:
            ...
            print(
                f"  {filename:<30} │ {format_tokens(tokens):>7} │ {DIM}{trigger}{RESET}"
            )

    print(f"\n{DIM}{'─' * 63}{RESET}")
    print(
        f"{BOLD}{MAGENTA}  ON-DEMAND RESERVE TOTAL{' ' * 10} │ {format_tokens(on_demand_total):>8} tokens{RESET}"
    )
    print(f"{DIM}{'─' * 63}{RESET}\n")
