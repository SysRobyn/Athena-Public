...
MONITORED_FILES = [
    ...
    ".framework/modules/Output_Standards.md",
...
def main():
    ...
    print(
        f"{BOLD}{CYAN}              🔍 CODEX WATCHDOG                                 {RESET}"
    )
    print(
        f"{CYAN}═══════════════════════════════════════════════════════════════{RESET}\n"
    )
    ...
    for rel_path in MONITORED_FILES:
        ...
        status_str = (
            f"{color}{status}{RESET}" if status != "OK" else f"{DIM}{status}{RESET}"
        )
    ...
    for rel_dir in SCAN_DIRS:
        ...
        for filename, tokens, _ in results:
            ...
            if status != "OK":
                issues_found += 1
                print(
                    f"     {emoji} {filename:<30} │ {tokens:>6,} tokens  [{color}{status}{RESET}]"
                )
            else:
    ...
    # Threshold legend
    print(
        f"{DIM}Thresholds: 🟢 <{THRESHOLDS['warning']:,} │ 🟡 <{THRESHOLDS['critical']:,} │ 🟠 <{THRESHOLDS['danger']:,} │ 🔴 ≥{THRESHOLDS['danger']:,} tokens{RESET}\n"
    )
