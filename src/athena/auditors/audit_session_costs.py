...
BOOT_FILE_PATH = os.path.join(PROJECT_ROOT, ".framework", "modules", "Core_Identity.md")
...
def analyze_sessions():
    files = sorted(
        glob.glob(os.path.join(SESSION_LOG_DIR, "*.md"))
        + glob.glob(os.path.join(SESSION_LOG_DIR, "archive", "*.md"))
    )
    ...
    print(
        f"{BOLD}{'Session':<30} | {'Size (Tokens)':<15} | {'Est. Cost (Monolith)':<20} | {'Est. Cost (Adaptive)':<20}{RESET}"
    )
    print(f"{CYAN}{'-' * 80}{RESET}")
    ...
    for log_path in files:
        ...
        avg_ctx_adaptive = (
            ADAPTIVE_BOOT + 5000 + (session_tokens / 2)
        )  # Boot + 1 module + growth

        cost_monolith = (
            avg_ctx_monolith * estimated_turns / 1_000_000
        ) * PRICE_PER_M_INPUT
        cost_adaptive = (
            avg_ctx_adaptive * estimated_turns / 1_000_000
        ) * PRICE_PER_M_INPUT
    ...
    print(
        f"  {BOLD}TOTAL SAVINGS:{RESET}              {GREEN}${total_monolith_cost - total_adaptive_cost:.2f} (-{((total_monolith_cost - total_adaptive_cost) / total_monolith_cost) * 100:.1f}%){RESET}"
    )
    print(
